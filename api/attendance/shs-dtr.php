...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        ...
        if ($periodStart && $periodEnd) {
            $stmt = $pdo->prepare("SELECT * FROM attendance_shs_dtr WHERE period_start = :start AND period_end = :end");
            $stmt->execute([':start' => $periodStart, ':end' => $periodEnd]);
        } else {
            $stmt = $pdo->query("SELECT * FROM attendance_shs_dtr");
        }
    ...
    case 'POST':
        ...
        $stmt = $pdo->prepare("SELECT id, daily_data FROM attendance_shs_dtr WHERE employee_id = :id AND period_start = :start AND period_end = :end");
        ...
        if ($existing) {
            $update = $pdo->prepare("UPDATE attendance_shs_dtr SET daily_data = :data, total_hours = :total, updated_at = CURRENT_TIMESTAMP WHERE id = :id");
            $update->execute([':data' => json_encode($dailyData), ':total' => $totalHours, ':id' => $existing['id']]);
        } else {
            $insert = $pdo->prepare("INSERT INTO attendance_shs_dtr (employee_id, period_start, period_end, daily_data, total_hours) VALUES (:id, :start, :end, :data, :total)");
