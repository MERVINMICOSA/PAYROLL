...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        ...
        if ($periodStart && $periodEnd) {
            $stmt = $pdo->prepare("SELECT * FROM attendance_sa WHERE period_start = :start AND period_end = :end");
    ...
    case 'POST':
        ...
        if (isset($input['rate'])) {
            $stmt = $pdo->prepare("SELECT id FROM attendance_sa WHERE employee_id = :id AND period_start = :start AND period_end = :end");
            $stmt->execute([':id' => $employeeId, ':start' => $periodStart, ':end' => $periodEnd]);
            $existing = $stmt->fetch(PDO::FETCH_ASSOC);
            
            if ($existing) {
                $update = $pdo->prepare("UPDATE attendance_sa SET rate = :rate, updated_at = CURRENT_TIMESTAMP WHERE id = :id");
                $update->execute([':rate' => $input['rate'], ':id' => $existing['id']]);
            } else {
                $insert = $pdo->prepare("INSERT INTO attendance_sa (employee_id, period_start, period_end, rate, daily_data, days_worked, total_pay) VALUES (:id, :start, :end, :rate, '{}', 0, 0)");
        ...
        $stmt = $pdo->prepare("SELECT id, daily_data, rate FROM attendance_sa WHERE employee_id = :id AND period_start = :start AND period_end = :end");
        $stmt->execute([':id' => $employeeId, ':start' => $periodStart, ':end' => $periodEnd]);
        $existing = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        if ($existing) {
            $update = $pdo->prepare("UPDATE attendance_sa SET daily_data = :data, days_worked = :days, total_pay = :pay, updated_at = CURRENT_TIMESTAMP WHERE id = :id");
            $update->execute([':data' => json_encode($dailyData), ':days' => $daysWorked, ':pay' => $totalPay, ':id' => $existing['id']]);
        } else {
            $insert = $pdo->prepare("INSERT INTO attendance_sa (employee_id, period_start, period_end, daily_data, days_worked, total_pay, rate) VALUES (:id, :start, :end, :data, :days, :pay, 0)");
