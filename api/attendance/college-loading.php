...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        ...
        if ($periodStart && $periodEnd) {
            $stmt = $pdo->prepare("SELECT * FROM attendance_college_loading WHERE period_start = :start AND period_end = :end");
    ...
    case 'POST':
        ...
        $stmt = $pdo->prepare("SELECT id FROM attendance_college_loading WHERE employee_id = :id AND period_start = :start AND period_end = :end");
        ...
        if ($existing) {
            $update = $pdo->prepare("UPDATE attendance_college_loading SET subject = :subject, mon = :mon, tue = :tue, wed = :wed, thu = :thu, fri = :fri, sat = :sat, sun = :sun, updated_at = CURRENT_TIMESTAMP WHERE id = :id");
            $update->execute(array_merge($data, [':id' => $existing['id']]));
        } else {
            $insert = $pdo->prepare("INSERT INTO attendance_college_loading (employee_id, period_start, period_end, subject, mon, tue, wed, thu, fri, sat, sun) VALUES (:id, :start, :end, :subject, :mon, :tue, :wed, :thu, :fri, :sat, :sun)");
